...
const WithDrawReport = () => {
  ...
  const ApplicationProperties = createSelector(
    selectAppState,
    (app) => ({
      paymentReports: app.paymentReports,
      allUsers: app.allUsers,
    })
  );
  const { paymentReports, allUsers } = useSelector(ApplicationProperties);
  ...
  const loadReport = useCallback((user: string, start: string, end: string) => {
    ...
    dispatch(getPaymentReport(param));
  },[]);
            ...
            <CardContent className=' p-2 m-5 h-full'>
              {paymentReports && paymentReports.length > 0?
                <DataTable
                  ...
                  content = {{
                    data: paymentReports,
                    totalItems: paymentReports.length,
                    currentPage: 1,
                    totalPages: 1
                  }}
                  columns={columns}
